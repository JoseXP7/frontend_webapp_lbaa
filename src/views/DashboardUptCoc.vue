...
const getUnCocinero = async () => {
  if (search.value == '') {
    ...
    try {
      const response = await api.get(`/cocineros/${Number(search.value)}`)
      cocineros.value = response.data.body
    } catch (error) {
      console.log(error)
    }
...
const getUniqueCocModal = async (ci) => {
  try {
    const response = await api.get(`/cocineros/${ci}`)
    uqCocinero.value = response.data.body
                    ...
                    <td>
                      ...
                      >
                        <button
                          ...
                          @click="getUniqueCocModal(cocinero.cedula)"
                        >
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div col-4>
              <ul
                class="list-group"
                v-for="cocinero in uqCocinero"
                :key="cocinero.id"
              >
                <li class="list-group-item">
                  <h4>ID: {{ cocinero.id }}</h4>
                </li>
                <li class="list-group-item">
                  Cedula:
                  <input
                ...
                <li class="list-group-item">
                  ...
                  <button
                    ...
                    @click="updateCocinero(cocinero.id)"
                    data-bs-dismiss="modal"
                  >
                    Actualizar
                  </button>
