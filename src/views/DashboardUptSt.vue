...
const getUnEstudiante = async () => {
  if (search.value == '') {
    ...
    try {
      const response = await api.get(`/estudiantes/${Number(search.value)}`)
      estudiantes.value = response.data.body
    } catch (error) {
      console.log(error)
    }
...
const getUniqueStModal = async (ci) => {
  try {
    ...
    uqEstudiante.value = response.data.body
...
const updateStudent = async (id) => {
  try {
    const response = await api.put(`/estudiantes`, {
      id: id,
      cedula: cedula.value,
                    ...
                    <td>
                      ...
                      >
                        <button
                          ...
                          @click="getUniqueStModal(estudiante.cedula)"
                        >
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div col-4>
              <ul
                class="list-group"
                v-for="estudiante in uqEstudiante"
                :key="estudiante.id"
              >
                ...
                <li class="list-group-item">
                  ...
                  <button
                    ...
                    @click="updateStudent(estudiante.id)"
                    data-bs-dismiss="modal"
                  >
                    Actualizar
                  </button>
